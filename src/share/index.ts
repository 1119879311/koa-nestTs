...
import { IGuard, ResigerRouters } from "./routerDecorator";
import {
  HttpExceptionFilter,
  IExceptionsFilter,
} from "./Filter/http.exceptions.filter";
import {
  httpResponseInterceptor,
  IResponseInterceptor,
} from "./Interceptor/http.response.interceptor";
...
export class KoaNestTs<T> {
  ...
  private guardQuence: Array<IGuard> = [];

  private pipesQuence: Array<Pipe> = [];

  private exceptionsFilterQuence: Array<IExceptionsFilter> = [
    HttpExceptionFilter,
  ];
  private responseInterceptorQuence: Array<IResponseInterceptor> = [
    httpResponseInterceptor,
  ];
  constructor(appModule: Type<T>, option?: KoaNestTsOpt) {
    this.Ioc = new Container();
    this.Ioc.init(appModule);
    this.koaApp = new Koa();
    this.koaRouter = new KoaRouter({ prefix: option?.routerPrefix });
  }
  ...
  public setGlobalPip(...fn: Array<Pipe>) {
    this.pipesQuence = [...this.pipesQuence, ...fn];
  }
  // 添加全局响应拦截器
  ...
  private setfirstMiddlWare() {
    ...
    const responseInterceptorMiddlWare = async (
  ...
  appError() {
    this.koaApp.on("error", (err, ctx) =>
      this.exceptionsFilterQuence.forEach((itme) => itme(ctx, err))
    );
  }
