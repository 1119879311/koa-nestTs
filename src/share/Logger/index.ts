    ...
    static write(str:string,levev:enumLeve){
        try {
            if(this.config[levev].write){
                ...
                loggerDir = loggerDir?`${loggerRoot}${loggerDir}`:`${loggerRoot}${levev}-${dateFormat({patter: 'YYYY-MM-DD'})}.log`
...
async function mkdirSync(dirname:string) {
    try {
