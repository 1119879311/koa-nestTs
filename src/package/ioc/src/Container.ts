...
export class ContainerBase {
  ...
  protected handleProviderForwardRef<T>(providerIforwardRef: IforwardRef<T>, provide?: Token<T>) {
    ...
    if (isFun(providerIforwardRef)) {
      ...
      if (isFun(resRroviderValue) && isNotBaseType(resRroviderValue.name)) {
        // 如果是一个类，要进行实例化
        // 创建依赖循环的缓存数据
  ...
  //获取provider 的值
  protected getProviderForwardRef<T = any>(provider: Provider<T>) {
    if (isClassProvider(provider)) {
  ...
  protected LoadForwardProviders() {
    this.instancesforwardMap.forEach((value: any, key: any) => {
  ...
  protected getInjectConstructParams<T>(target: Type<T>): any[] {
    ...
    return args.map((itme: any, index: number) => {
      ...
      // 参数类型，有可能是forwarkRef
      if (isforwardRef(paramsToken) && isFun(paramsToken)) {
        // 如果是 循环依赖，从实例种取值
      ...
      let provider = this.providers.get(paramsToken) as Provider<T>;
  ...
  protected assetInjectableIsClassProvide<T>(provider: ClassProvider<T>) {
    if (isClassProvider(provider) && !(provider.provide instanceof InjectToken) && !isInjectable(provider.useClass)) {
      let errmsg = `cannot  Provider (${getTokenName(provider.provide)}) using useClass (${getTokenName(
  ...
  protected assetClassProvideEqual(provide: any, useClass: Type<any>) {
    if (isFun(provide) && isFun(useClass) && provide !== useClass) {
      throw new Error(
        `When the type of provider is ClassProvider and the type of provider and useClass is Class, the provide ${provide.name} and useClass ${useClass.name} must be equal ,but the results are not equal`,
      );
    }
  }
}
...
export class Container<K> extends ContainerBase {
  ...
  public bindModule<T>(module: Type<T>) {
    if (!isModule(module)) {
      throw new Error(
        ` cannot  imports [${getTokenName(module)}] using  [${getTokenName(module)}]， ${getTokenName(
          module,
        )} is not module `,
      );
    }
